...
public class Tela_Aluguer


extends JFrame {
	private JTable tabela_aluguer;
    private CampoTexto txt_pesquisa, txt_pesq_aluguer,txt_aluguer_sel;
    ...
    private JScrollPane pane_tabela_aluguer;
    private Tabela_Aluguer model;
    ...
    private void inicializacao() {
      ...
      //inicializacao do campo pesquisa de um  funcionario
        txt_pesq_aluguer = new CampoTexto();
        txt_pesq_aluguer .setTextoFundo("Digite o ID Aluguer");
        txt_pesq_aluguer .setFont(new Font("Century Gothic", Font.BOLD, 12));
        txt_pesq_aluguer .setForeground(new Color(29, 36, 98));
        pane_menu_lateral.add( txt_pesq_aluguer );
        ...
        txt_aluguer_sel= new CampoTexto();
        txt_aluguer_sel.setEditable(false);
        txt_aluguer_sel.setBackground(new Color(240, 240, 240));
        txt_aluguer_sel.setForeground(new Color(80, 80, 80));
        txt_aluguer_sel.setFont(new Font("Centuary Gothic", Font.BOLD, 13));
        pane_menu_lateral.add(txt_aluguer_sel);
      ...
      //inicializacao do painel de botoes(botoes que fazem CRUD da Viatura)
        ...
        pane_tabela_aluguer = new JScrollPane();
        pane_tabela_aluguer.setBackground(cor_font_botao);
        ...
        tabela_aluguer.setRowHeight(25);
        pane_tabela_aluguer.setViewportView(tabela_aluguer);
        pane_conteudo.add(pane_tabela_aluguer);
    ...
    public void layouts() {
     ...
     // localizacao do campo pesquisa de uma determinada viatura pela(matricula)
        txt_pesq_aluguer.setBounds(10, 80, 280, 25);
        
     // definicao da localizacao da tabela consulta
        h = pane_menu_lateral.getHeight() - 200;
        y = txt_pesq_aluguer.getHeight() + txt_pesq_aluguer.getY() + 10;
       ...
       int y1=h+y+10;
        txt_aluguer_sel.setBounds(10, y1, 280, 27);
        ...
        pane_tabela_aluguer.setBounds(10, y, w, h);
        tabela_aluguer.setBounds(0, 0, w, h);
    ...
    public void preencherTabela() {
    	...
    	List<Aluguer> lista= aluguerDAO.listar();
        model = new Tabela_Aluguer(lista);
        tabela_aluguer.setModel(model);

        DefaultTableCellRenderer render = new DefaultTableCellRenderer();
        render.setHorizontalAlignment(SwingConstants.CENTER);

        for (int i = 0; i < model.getColumnCount(); i++) {
        	tabela_aluguer.getColumnModel().getColumn(i).setCellRenderer(render);
        }
    }
    private void eventosButoes() {
        ...
        btn_novo.addActionListener(new ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
               Cadastro_Aluguer cada=new Cadastro_Aluguer(); 
               cada.dispose();
               cada.show();
               }
      });
        ...
        btn_editar.addActionListener(new ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
               Actualizar_Aluguer actualizar=new Actualizar_Aluguer(); 
               actualizar.dispose();
               actualizar.show();
               }
      });
        ...
        btn_remover.addActionListener(new ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
               Remover_Aluguer remover=new Remover_Aluguer(); 
               remover.dispose();
               remover.show();
               }
      });
