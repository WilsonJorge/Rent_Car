...
extends JFrame {
	private JTable tabela_cliente;
    private CampoTexto txt_pesquisa, txt_pesq_cliente,txt_cliente_sel;
    ...
    private JScrollPane pane_tabela_cliente;
    private Tabela_Cliente model;
    ...
    private void inicializacao() {
      ...
      //inicializacao do campo pesquisa de um  funcionario
        txt_pesq_cliente = new CampoTexto();
        txt_pesq_cliente.setTextoFundo("Digite o ID do Cliente");
        txt_pesq_cliente.setFont(new Font("Century Gothic", Font.BOLD, 12));
        txt_pesq_cliente.setForeground(new Color(29, 36, 98));
        pane_menu_lateral.add(txt_pesq_cliente);

        //inicializacao do scrolpane para tabela Viatura
        pane_tabela_cliente= new JScrollPane();
        pane_tabela_cliente.setBackground(Color.WHITE);
        pane_menu_lateral.add(pane_tabela_cliente);

        //inicializacao da tabela Viatura
        tabela_cliente = new JTable();
        tabela_cliente.setBackground(new Color(255, 255, 255));
        tabela_cliente.setFont(new Font("Centuary Gothic", Font.BOLD, 13));
        tabela_cliente.setGridColor(new Color(75, 130, 181));
        tabela_cliente.getTableHeader().setFont(new Font("Centuary Gothic", Font.BOLD, 15));
        tabela_cliente.getTableHeader().setBackground(new Color(160, 196, 229));
        tabela_cliente.getTableHeader().setForeground(new Color(29, 36, 98));
        tabela_cliente.setRowHeight(25);
        pane_tabela_cliente.setViewportView(tabela_cliente);

        //inicializacao da caixa do texto para mostara a viatura  selecionada
        txt_cliente_sel= new CampoTexto();
        txt_cliente_sel.setEditable(false);
        txt_cliente_sel.setBackground(new Color(240, 240, 240));
        txt_cliente_sel.setForeground(new Color(80, 80, 80));
        txt_cliente_sel.setFont(new Font("Centuary Gothic", Font.BOLD, 13));
        pane_menu_lateral.add(txt_cliente_sel);
        
        //inicializacao do botao para listar o carro  selecionado
        btn_listar = new Botao("Cliente Selecionado");
      ...
      //inicializacao do painel de botoes(botoes que fazem CRUD da Viatura)
        ...
        txt_pesquisa.setTextoFundo("Digite aqui para pesquisar um Cliente");
        ...
        pane_tabela_cliente = new JScrollPane();
        pane_tabela_cliente.setBackground(cor_font_botao);
        ...
        tabela_cliente.setRowHeight(25);
        pane_tabela_cliente.setViewportView(tabela_cliente);
        pane_conteudo.add(pane_tabela_cliente);
    ...
    public void layouts() {
     ...
     // localizacao do campo pesquisa de uma determinada viatura pela(matricula)
        txt_pesq_cliente.setBounds(10, 80, 280, 25);
        
     // definicao da localizacao da tabela consulta
        h = pane_menu_lateral.getHeight() - 200;
        y = txt_pesq_cliente.getHeight() + txt_pesq_cliente.getY() + 10;
        pane_tabela_cliente.setBounds(10, y, 280, h);
        tabela_cliente.setSize(pane_tabela_cliente.getSize());
     

       //definicao da localizacao campo viatura selecionadas
       int y1=h+y+10;
        txt_cliente_sel.setBounds(10, y1, 280, 27);
        ...
        pane_tabela_cliente.setBounds(10, y, w, h);
        tabela_cliente.setBounds(0, 0, w, h);
  ...
  //Metodo para fazer Tabela 
    public void preencherTabela() {
        ...
        model = new Tabela_Cliente(lista);
        tabela_cliente.setModel(model);

        DefaultTableCellRenderer render = new DefaultTableCellRenderer();
        render.setHorizontalAlignment(SwingConstants.CENTER);

        for (int i = 0; i < model.getColumnCount(); i++) {
        	tabela_cliente.getColumnModel().getColumn(i).setCellRenderer(render);
        }
    }
    ...
    private void eventosButoes() {
        ...
        btn_editar.addActionListener(new ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
               Actualizar_Cliente tela=new Actualizar_Cliente(); 
               tela.dispose();
               tela.show();
               }
        ...
        btn_remover.addActionListener(new ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
               Remover_Cliente tela=new Remover_Cliente(); 
               tela.dispose();
               tela.show();
               }
