...
public class Tela_Funcionario extends JFrame 
{
    ...
    private CampoTexto txt_pesquisa, txt_pesq_funcionario,txt_funcionario_sel;
    ...
    private Tabela_Funcionario model;
    ...
    private void inicializacao() {
      ...
      //inicializacao do campo pesquisa de um  funcionario
        txt_pesq_funcionario = new CampoTexto();
        txt_pesq_funcionario.setTextoFundo("Digite o ID do Funcionario");
        txt_pesq_funcionario.setFont(new Font("Century Gothic", Font.BOLD, 12));
        txt_pesq_funcionario.setForeground(new Color(29, 36, 98));
        pane_menu_lateral.add(txt_pesq_funcionario);

        //inicializacao do scrolpane para tabela Viatura
        pane_tabela_funcionario= new JScrollPane();
        pane_tabela_funcionario.setBackground(Color.WHITE);
        pane_menu_lateral.add(pane_tabela_funcionario);

        //inicializacao da tabela Viatura
        tabela_funcionario = new JTable();
        tabela_funcionario.setBackground(new Color(255, 255, 255));
        tabela_funcionario.setFont(new Font("Centuary Gothic", Font.BOLD, 13));
        tabela_funcionario.setGridColor(new Color(75, 130, 181));
        tabela_funcionario.getTableHeader().setFont(new Font("Centuary Gothic", Font.BOLD, 15));
        tabela_funcionario.getTableHeader().setBackground(new Color(160, 196, 229));
        tabela_funcionario.getTableHeader().setForeground(new Color(29, 36, 98));
        tabela_funcionario.setRowHeight(25);
        pane_tabela_funcionario.setViewportView(tabela_funcionario);

        //inicializacao da caixa do texto para mostara a viatura  selecionada
        txt_funcionario_sel= new CampoTexto();
        txt_funcionario_sel.setEditable(false);
        txt_funcionario_sel.setBackground(new Color(240, 240, 240));
        txt_funcionario_sel.setForeground(new Color(80, 80, 80));
        txt_funcionario_sel.setFont(new Font("Centuary Gothic", Font.BOLD, 13));
        pane_menu_lateral.add(txt_funcionario_sel);
        
        //inicializacao do botao para listar o carro  selecionado
        btn_listar = new Botao("Funcionario Selecionado");
        ...
        btn_listar.setBackgroundMouseEntered(new Color(51, 71, 72));
      ...
      //inicializacao do painel de botoes(botoes que fazem CRUD da Viatura)
        ...
        pane_tabela_funcionario = new JScrollPane();
        pane_tabela_funcionario.setBackground(cor_font_botao);
        ...
        tabela_funcionario.setRowHeight(25);
        pane_tabela_funcionario.setViewportView(tabela_funcionario);
        pane_conteudo.add(pane_tabela_funcionario);
    ...
    public void layouts() {
     ...
     // localizacao do campo pesquisa de uma determinada viatura pela(matricula)
        txt_pesq_funcionario.setBounds(10, 80, 280, 25);
        
     // definicao da localizacao da tabela consulta
        h = pane_menu_lateral.getHeight() - 200;
        y = txt_pesq_funcionario.getHeight() + txt_pesq_funcionario.getY() + 10;
        pane_tabela_funcionario.setBounds(10, y, 280, h);
        tabela_funcionario.setSize(pane_tabela_funcionario.getSize());
     

       //definicao da localizacao campo viatura selecionadas
       int y1=h+y+10;
        txt_funcionario_sel.setBounds(10, y1, 280, 27);
        ...
        pane_tabela_funcionario.setBounds(10, y, w, h);
        tabela_funcionario.setBounds(0, 0, w, h);
  ...
  //Metodo para fazer Tabela 
    public void preencherTabela() {
        ...
        model = new Tabela_Funcionario(lista);
        tabela_funcionario.setModel(model);
        ...
        for (int i = 0; i < model.getColumnCount(); i++) {
        	tabela_funcionario.getColumnModel().getColumn(i).setCellRenderer(render);
        }
    }
    ...
    private void eventosButoes() {
       ...
       //terminado 
        btn_novo.addActionListener(new ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
               Cadastro_Funcionario cada=new Cadastro_Funcionario(); 
               //cada.dispose();
               cada.show();
               }
      });
        ...
        btn_editar.addActionListener(new ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
               Actualizar_Funcionario actualizar=new Actualizar_Funcionario(); 
               actualizar.show();
               }
        ...
        btn_remover.addActionListener(new ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
               Remover_Funcionario remover=new Remover_Funcionario(); 
               //remover.dispose();
               remover.show();
               }
